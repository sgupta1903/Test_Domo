...
import static helper.DataProviderSection.CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD;
...
@Issue("A30TP-21993")
@Slf4j
public class CreateCurrentAutoRenewToOpenSubsSemiAnnuallyWithDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay("A30TP-14514");
        set_section_name(CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD);
    }
    ...
    public void enterMemberDetailsForFirstLocation() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,
                        SUB_DOWNPAYMENT));
    }
    ...
    public void enterRenewToOpenSubscription() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT));
    }

    @Test
    @Description("Create two Subscriptions autorenew to open type with semi-annual frequency when begin date is current date for 1st location with one organization when payment is done by Card with down payment")
    @TmsLink("A30TP-31976")
    public void createNonRenewSubscription() {
        log.info("Create two Subscriptions autorenew to open  type with semi-annual frequency when begin date is current date for 1st location with one organization when payment is done by Card with down payment");
        setTestResultOnXRay("A30TP-31976");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_renewal_auto_to_open(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD, SUB_RENEWAL_TYPE,
                        SUB_RENEWAL_DATE, SUB_RENEWAL_FREQUENCY));
    }

    @Test
    @Description("Create two Subscriptions autorenew to open type with semi-annual frequency when begin date is current date and expiration and due date for 1st location with one organization when payment is done by Card with down payment")
    @TmsLink("A30TP-31979")
    public void enterDetailsForSubscription() {
        log.info("Create two Subscriptions autorenew to open type with semi-annual frequency when begin date is current date and expiration and due date for 1st location with one organization when payment is done by Card with down payment");
        setTestResultOnXRay("A30TP-31979");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,SUB_CARD_HOLDER_NAME));
    }
    ...
    @Test
    @Description("Create two Subscriptions auto-renew to open type with semi-annual frequency when begin date is current date for 2nd location with one organization when payment is done by Card with down payment")
    @TmsLink("A30TP-31984")
    public void enterSubscriptionForSecondLocation() {
        log.info("Create two Subscriptions auto-renew to open type with semi-annual frequency when begin date is current date for 2nd location with one organization when payment is done by Card with down payment");
        setTestResultOnXRay("A30TP-31984");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information_for_second_location(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,
    ...
    @Test
    @Description("Create two Subscriptions auto-renew to open type with semi-annual frequency when begin date is current date and expiration and due date for 2nd location with one organization when payment is done by Card with down payment")
    @TmsLink("A30TP-31985")
    public void enterCardDetails() {
        log.info("Create two Subscriptions auto-renew to open type with semi-annual frequency when begin date is current date and expiration and due date for 2nd location with one organization when payment is done by Card with down payment");
        setTestResultOnXRay("A30TP-31985");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,
                        SUB_CARD_NUMBER, SUB_CARD_HOLDER_NAME, SUB_CARD_EXPIRY_YEAR, SUB_CARD_POSTAL_CODE))
                .click_on_submit_btn()
    ...
    public void verifySubscriptionTabForSecondLcoation() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .select_subscription_tab()
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity_second_location_item(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,SECOND_ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_LOCATION_WITHDWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_CARD,SUB_TYPE));
    }
