package testcase.unoapptestcases.endtoendscenario.scenario.openended.card;
...
import static helper.DataProviderSection.FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD;
...
public class CreateFutureSubsBiWeeklyWithOutDwnPay_Test extends AbstractAutoUITest {
    ...
    public void setSectionData() {
        ...
        set_section_name(FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD);
    }
    ...
    public void configuringMerchantVantivProcessor() {
        logger.info("Create merchant account with vantiv processor.");
    ...
    @Test
    @Description("Enter Payment Data while creating subscription")
    public void enterPaymentInfo() {
        logger.info("Creation of Subscription has started");
        setTestResultOnXRay("A30TP-8173");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD,
                        SUB_DOWNPAYMENT))
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT));
    }

    @Test
    @Description("Create 'Open-End' Subscription with Payment Method Type as 'Credit Card' with begin date future when payment is done with frequency Bi-weekly")
    public void enterScheduleInfo() {
        logger.info("Create 'Open-End' Subscription with Payment Method Type as 'Credit Card' with begin date future when payment is done with frequency Bi-weekly");
        setTestResultOnXRay("A30TP-8175");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD,
                        SUB_CARD_NUMBER, SUB_CARD_HOLDER_NAME, SUB_CARD_EXPIRY_YEAR, SUB_CARD_POSTAL_CODE))
                .click_on_submit_btn();
    }

    @Test
    @Description("Create Subscription with member for OPEN-ENDED")
    public void createSubsAutoRenewToTermType() {
        logger.info("Verifying the subscription is created successfully.");
        setTestResultOnXRay("A30TP-8176");
        getPage(UnoAppSubscriptionPage.class)
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,FUTURE_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_CARD,SUB_CARD_HOLDER_NAME));
    }
