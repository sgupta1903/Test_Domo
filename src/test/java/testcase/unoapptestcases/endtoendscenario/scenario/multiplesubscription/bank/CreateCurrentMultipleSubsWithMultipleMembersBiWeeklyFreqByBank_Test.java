...
import pagetest.paymentgatewaypage.*;
...
@Issue("A30TP-13124")
@Slf4j
public class CreateCurrentMultipleSubsWithMultipleMembersBiWeeklyFreqByBank_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay("A30TP-14514");
        set_section_name(MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK);
    }
    ...
    @Test
    @Description("Create multiple subscription for two members where one subscription is open-ended and frequency is bi-weekly and begin date is current where payment method as bank account.")
    @TmsLink("A30TP-18583")
    public void enterDetailsForSubscription() {
        log.info("Create multiple subscription for two members where one subscription is open-ended and frequency is bi-weekly and begin date is current where payment method as bank account. ");
        setTestResultOnXRay("A30TP-18583");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK,
                        SUB_DOWNPAYMENT))
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK, SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK, ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK, SUB_TYPE));
    }

    @Test
    @Description("Create multiple subscription for two members where one subscription is non-renewal installment and frequency is bi-weekly and begin date is current where payment method as bank account.")
    @TmsLink("A30TP-18584")
    public void createSecondSuscription() {
        log.info("Create multiple subscription for two members where one subscription is non-renewal installment and frequency is bi-weekly and begin date is current where payment method as bank account.");
        setTestResultOnXRay("A30TP-18584");
        getPage(UnoAppMemberDetailPage.class)
                .enter_installment_another_subscription_details(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK,
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK, PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_OPEN_ENDED_BI_WEEKLY_BANK, SUB_ACCOUNT_HOLDER_NAME));
