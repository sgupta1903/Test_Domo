...
import static helper.DataProviderSection.MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK;
...
@Issue("A30TP-13121")
@Slf4j
public class CreatePastMultipleSubsAnnuallyWithDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay("A30TP-14514");
        set_section_name(MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK);
    }
    ...
    @Test
    @Description("Create multiple subscription when one member is getting open-ended subscription and other is installment non-renewal subscription with annually frequency where begin and due date is past with bank account")
    @TmsLink("A30TP-38339")
    public void enterDetailsForSubscription() {
        log.info("Create multiple subscription when one member is getting open-ended subscription and other is installment non-renewal subscription with annually frequency where begin and due date is past with bank account");
        setTestResultOnXRay("A30TP-38339");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK,
                        SUB_DOWNPAYMENT))
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK,
                        SUB_ACCOUNT_HOLDER_NAME, SUB_BANK_ROUTING_NUMBER, SUB_BANK_ACCOUNT_NUMBER))
                .click_on_submit_btn();
    }

    @Test
    @Description("Creating Second member and fill all the details in information page")
    @TmsLink("A30TP-37781")
    public void addSecondMember() {
        log.info("Creating Second member and fill all the details in information page");
        setTestResultOnXRay("A30TP-37781");
        set_section_name(MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK);
        getPage(UnoAppMemberPage.class)
                .navigate_and_click_add_member_btn()
                .generate_member_first_name(MEMBER_FIRST_NAME)
                .enter_member_details(getTestData(UNOAPP_INI, MEMBER_SECTION, MEMBER_ADDRESS, MEMBER_NUMBER,
                        MEMBER_FIRST_NAME, MEMBER_LAST_NAME, MEMBER_CITY, MEMBER_STATE, MEMBER_COUNTRY, MEMBER_ZIP_CODE, MEMBER_PHONE_NUMBER))
                .click_continue_button()
                .create_payor();
    }
    ...
    public void createSecondSubscription() {
        log.info("Create multiple subscription when one member is getting open-ended subscription and other is installment non-renewal subscription with annually frequency where begin and due date is past with bank account");
        setTestResultOnXRay("A30TP-38341");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK,
                        SUB_DOWNPAYMENT))
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK, SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK, ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK, SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK, PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITH_DWNPAY_OPENENDED_ANNUALLY_BANK, SUB_ACCOUNT_HOLDER_NAME));
