...
import static helper.DataProviderSection.MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD;
import static helper.DataProviderSection.PAST_MULTIPLESUBSCRIPTION_NONRENEW_MONTHLY_CARD_INVALID_PROCESSOR;
...
@Issue("A30TP-13093")
@Slf4j
public class CreatePastSubsSemiAnnuallyWithoutDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD);
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create multiple 'Installment' subscription for one member with payment method type as credit card when begin,first due date is past where frequency is Semi-Annually ")
    @TmsLink("A30TP-37814")
    public void createOpenEndedSubscription() {
        log.info("Create multiple 'Installment' subscription for one member with payment method type as credit card when begin,first due date is past where frequency is Semi-Annually");
        setTestResultOnXRay("A30TP-37814");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD,
    ...
    public void addPaymentDetailsForSubscription() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, PAST_MULTIPLESUBSCRIPTION_NONRENEW_MONTHLY_CARD_INVALID_PROCESSOR,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD, SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD, ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD, SUB_TYPE));
    }

    @Test
    @Description("Create multiple subscription, other subscription installment with non-renew agreement where frequency is monthly first due date and begin date is current and expiration date is future payment method as credit card")
    @TmsLink("A30TP-37825")
    public void createAnotherSubs() {
        log.info("Create multiple subscription, other subscription installment with non-renew agreement where frequency is monthly first due date and begin date is current and expiration date is future payment method as credit card");
        setTestResultOnXRay("A30TP-37825");
        getPage(UnoAppMemberDetailPage.class)
                .enter_installment_another_subscription_details(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD,
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_NON_RENEW_SEMIANNUALLY_CARD, SUB_CARD_HOLDER_NAME));
    }
