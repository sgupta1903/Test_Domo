...
import static helper.DataProviderSection.MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD;
import static util.UtilityGeneric.set_section_name;

@Issue("A30TP-33398")
@Slf4j
public class CreateCurrentMultipleSubsBIWeeklyWithoutDwnPayWithAmex_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD);
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create multiple open ended subscription for one member with Payment Method Type as 'Credit Card' when begin date is current date where frequency is annually")
    @TmsLink("A30TP-17381")
    public void createOpenEndedSubscription() {
        log.info("Create multiple open ended subscription for one member with Payment Method Type as 'Credit Card' when begin date is current date where frequency is annually");
        setTestResultOnXRay("A30TP-17381");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD,
    ...
    public void createSecondSubscription() {
        log.info("Create multiple open ended subscription for one member with Payment Method Type as 'Credit Card' when begin date is current date where frequency is annually");
        setTestResultOnXRay("A30TP-36451");
        getPage(UnoAppMemberDetailPage.class)
                .select_subscription_tab()
                .enter_open_ended_another_subscription_details(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD, SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD, ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD, SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_amex(getTestData(UNOAPP_INI, MEMBER_SECTION, PAYMENT_METHOD_AMEX))
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITHOUT_DWNPAY_OPENENDED_BIWEEKLY_AMEX_CARD, SUB_CARD_HOLDER_NAME));
    }
