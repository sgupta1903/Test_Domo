...
import static helper.DataProviderSection.MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK;
...
@Issue("A30TP-13123")
@Slf4j
public class CreateMultipleSubsWeeklyWithDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay("A30TP-14514");
        set_section_name(MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK);
    }
    ...
    @Test
    @Description("Create multiple subscription when one member is getting open-ended subscription and other is installment non-renewal subscription with weekly frequency where begin and due date is current with bank account")
    @TmsLink("A30TP-38375")
    public void enterDetailsForSubscription() {
        log.info("Create multiple subscription when one member is getting open-ended subscription and other is installment non-renewal subscription with weekly frequency where begin and due date is current with bank account");
        setTestResultOnXRay("A30TP-38375");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK,
                        SUB_DOWNPAYMENT))
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK, SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK, ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK, SUB_TYPE));
    }
    ...
    public void createSecondSubscription() {
        log.info("Create multiple subscription when one member is getting open-ended subscription and other is installment non-renewal subscription with weekly frequency where begin and due date is current with bank account");
        setTestResultOnXRay("A30TP-38384");
        getPage(UnoAppMemberDetailPage.class)
       .enter_installment_another_subscription_details(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK,
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK, PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_CURRENT_WITH_NON_RENEW_WEEKLYY_BANK, SUB_ACCOUNT_HOLDER_NAME));
    }
