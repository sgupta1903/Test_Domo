...
@Issue("A30TP-13262")
@Slf4j
public class CreatePastMultiSubsQuarterlyWithOutDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK);
    }
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create multiple Open ended subscription for one member with Quarterly frequency where begin date is Past with bank account")
    @TmsLink("A30TP-16015")
    public void createOpenEndedSubscription() {
        log.info("Create multiple Open ended subscription for one member with Quarterly frequency where begin date is Past with bank account");
        setTestResultOnXRay("A30TP-16015");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,SUB_TYPE));
    }
    ...
    public void createAnotherSubs() {
        log.info("Create multiple Open ended subscription for one member with Quarterly frequency where begin date is Past with bank account");
        setTestResultOnXRay("A30TP-16021");
        getPage(UnoAppMemberDetailPage.class)
                .enter_open_ended_another_subscription_details(getTestData(SUBSCRIPTION_INI, MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI,MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI,MULTIPLE_SUBS_PAST_WITHOUT_DWNPAY_OPEMENDED_QUARTERLY_BANK,SUB_ACCOUNT_HOLDER_NAME));
