...
import static helper.DataProviderSection.VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED;
...
@Issue("A30TP-21644")
@Slf4j
public class PaymentMethodChangesToTransferred extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED);
    }
    ...
    @TmsLink("A30TP-20723")
    public void configureationMerchantWithVantivAndEFT() {
        log.info("Configure VANTIV processor configuration for the selected location for Payment Gateway");
        setTestResultOnXRay("A30TP-20723");
        getPage(MerchantAccountPage.class)
                .create_merchant_for_vantiv_and_eft();
    }
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create 'Open-End' Subscription when begin date current when frequency is annually with down payment")
    @TmsLink("A30TP-12374")
    public void enterDetailsForSubscription() {
        log.info("Create 'Open-End' Subscription when begin date current when frequency is annually with down payment");
        setTestResultOnXRay("A30TP-12374");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,VERIFY_PAYMENT_METHOD_CHANGES_TO_TRANSFERRRED,SUB_CARD_HOLDER_NAME));
    }
