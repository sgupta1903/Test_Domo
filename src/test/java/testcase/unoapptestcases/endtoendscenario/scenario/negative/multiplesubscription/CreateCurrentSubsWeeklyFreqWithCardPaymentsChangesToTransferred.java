...
import static helper.DataProviderSection.CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD;
...
@Issue("A30TP-33815")
@Slf4j
public class CreateCurrentSubsWeeklyFreqWithCardPaymentsChangesToTransferred extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD);
    }
    ...
    @Test
    @Description("Create installment Subscription with non-renew type when begin date and expiration date is current and frequency is Weekly with Credit Card with amount $40 .")
    @TmsLink("A30TP-35128")
    public void enterDetailsForSubscription() {
        log.info("Create installment Subscription with non-renew type when begin date and expiration date is current and frequency is Weekly with Credit Card with amount $40 .");
        setTestResultOnXRay("A30TP-35128");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,SUB_TYPE));
    }

    @Test
    @Description("Create Open-ended subscription  when begin date date is current and frequency is Weekly with amount $20 .")
    @TmsLink("A30TP-35128")
    public void createAnotherSubs() {
        log.info("Create Open-ended subscription  when begin date date is current and frequency is Weekly with amount $20 .");
        setTestResultOnXRay("A30TP-35128");
        getPage(UnoAppMemberDetailPage.class)
                .enter_installment_another_subscription_details(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLE_SUBSCRIPTION_NON_RENEW_WEEKLY_TRANSFERRED_CARD,SUB_CARD_HOLDER_NAME));
    }

    @Test
    @Description("Create another payment method for member")
    @TmsLink("A30TP-33089")
    public void createAnothrerPaymentMethod() {
    ...
    public void verifyExistingPaymentStatusChangedToTransferred() {
        ...
        setTestResultOnXRay("A30TP-35132");
        getPage(UnoAppMemberDetailPage.class)
