...
import static helper.DataProviderSection.CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED;
import static util.UtilityGeneric.set_section_name;

@Issue("A30TP-33279")
@Slf4j
public class CreateCurrentSubsWeeklyFreqWithBankPaymentStatusChangesToTransferred_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay("A30TP-14514");
        set_section_name(CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED);
    }
    ...
    public void creatingMerchantAccountForSecondLocation() {
        ...
        getPage(MerchantAccountPage.class)
                .create_merchant_regions_bank_for_second_location(SECONDLOCATION);
    }
    ...
    @Test
    @Description("Enter schedule information for installment subscription for first location")
    @TmsLink("A30TP-37873")
    public void enterSubscriptionScheduleDetails() {
        log.info("Enter schedule information for installment subscription for first location");
        setTestResultOnXRay("A30TP-37873");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,SUB_ACCOUNT_HOLDER_NAME));
    ...
    @TmsLink("A30TP-12135")
    public void addSecondMemberForSecondLocation() {
    ...
    @Test
    @Description("Create subscription with second location where subscription type is installment with bank account where subscription with amount $40 and being date is current and expiration is future and frequency is Weekly without down payment..")
    @TmsLink("A30TP-37882")
    public void enterSubscriptionForSecondLcoation() {
        log.info("Create subscription with second location where subscription type is installment with bank account where subscription with amount $40 and being date is current and expiration is future and frequency is Weekly without down payment..");
        setTestResultOnXRay("A30TP-37882");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information_for_second_location(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,
    ...
    public void verifySubscriptionTabForSecondLocation() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .select_subscription_tab()
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity_second_location_item(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,SECOND_ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,SUB_TYPE));
    }
    ...
    @Test
    @Description("Create multiple location with one organization, for first location is getting normal subscription and for second subscription payment-method getting changes and payment-method status changes to transferred.")
    @TmsLink("A30TP-37881")
    public void createAnotherPaymentMethod() {
        log.info("Create multiple location with one organization, for first location is getting normal subscription and for second subscription payment-method getting changes and payment-method status changes to transferred.");
        setTestResultOnXRay("A30TP-37881");
        getPage(UnoAppMemberDetailPage.class)
                .click_on_add_credit_card_button();
        getPage(UnoAppSubscriptionPage.class)
                .enter_add_another_payment_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLELOCATION_NONRENEW_WEEKLY_BANK_TRANSFERRED,
                        SUB_CARD_NUMBER, SUB_CARD_HOLDER_NAME, SUB_CARD_EXPIRY_YEAR, SUB_CARD_POSTAL_CODE));
    }

    @Test
    @Description("Verifying Payment method is changed after new payment method is added.")
    @TmsLink("A30TP-32994")
    public void verifyExistingPaymentStatusChangedToTransferred() {
        log.info("Verifying Payment method is changed after new payment method is added and changed to transferred");
        setTestResultOnXRay("A30TP-32994");
        getPage(UnoAppMemberDetailPage.class)
                .select_payment_details();
         /*       .verify_the_payment_status(getTestData(UNOAPP_INI,MEMBER_SECTION,PAYMENT_METHOD_STATUS));
            .wait_until_payment_status_changed();*/
    }
    @Test
