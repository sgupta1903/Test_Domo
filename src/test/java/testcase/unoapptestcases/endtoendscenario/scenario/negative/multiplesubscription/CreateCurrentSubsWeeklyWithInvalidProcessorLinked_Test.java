...
import static helper.DataProviderSection.CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR;
...
@Issue("A30TP-33807")
@Slf4j
public class CreateCurrentSubsWeeklyWithInvalidProcessorLinked_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR);
    }
    ...
    @Test
    @Description("Create member with all required fields")
    @TmsLink("A30TP-8451")
    public void createSubsWithOneMember() {
        log.info("Create member with all required fields");
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create multiple subscription when subscription type is installment Subscription with non-renew agreement type when begin date and expiration date is current and frequency is Weekly with Credit Card with amount $40 .")
    @TmsLink("A30TP-38488")
    public void createNonRenewInstallmentSubscription() {
        log.info("Create multiple subscription when subscription type is installment Subscription with non-renew agreement type when begin date and expiration date is current and frequency is Weekly with Credit Card with amount $40 .");
        setTestResultOnXRay("A30TP-38488");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR,
    ...
    public void addPaymentDetailsForSubscription() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR, SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR, ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR, SUB_TYPE));
    }

    @Test
    @Description("Create multiple subscription when subscription type is installment Subscription with non-renew agreement type when begin date and expiration date is current")
    @TmsLink("A30TP-38490")
    public void createAnotherSubs() {
        log.info("Create multiple subscription when subscription type is installment Subscription with non-renew agreement type when begin date and expiration date is current");
        setTestResultOnXRay("A30TP-38490");
        getPage(UnoAppMemberDetailPage.class)
                .enter_installment_another_subscription_details(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR,
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI, CURRENT_MULTIPLESUBSCRIPTION_NONRENEW_WEEKLY_CARD_INVALID_PROCESSOR, SUB_CARD_HOLDER_NAME));
    }
