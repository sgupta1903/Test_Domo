...
import static helper.DataProviderSection.FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK;
...
public class CreateFutureSubsWeeklyWithDwnPay_Test extends AbstractAutoUITest {
    ...
    public void setSectionData() {
        ...
        set_section_name(FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK);
    }
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create auto-renewal to open subscription begin date is future date and frequency is WEEKLY when payment is done by bank account")
    public void createRenewToOpenSubscription() {
        logger.info("Create auto-renewal to open subscription begin date is future date and frequency is WEEKLY when payment is done by bank account");
        setTestResultOnXRay("A30TP-32371");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_renewal_auto_to_open(getTestData(SUBSCRIPTION_INI, FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK, SUB_RENEWAL_TYPE,
    ...
    public void enterDetailsForSubscription() {
        logger.info("Entering the details of  account ");
        setTestResultOnXRay("A30TP-32372");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,FUTURE_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_BANK,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_wallet_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI,CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_BANK,PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_BANK,SUB_ACCOUNT_HOLDER_NAME));
