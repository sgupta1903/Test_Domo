...
import static helper.DataProviderSection.CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD;
...
@Issue("A30TP-21947")
@Slf4j
public class CreateCurrentSubsWeeklyWithDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(DataProviderSection.CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD);
    }
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create Installment subscription with credit card where subscription type is auto-renew to open and being date is current date and frequency is WEEKLY with down payment.")
    @TmsLink("A30TP-21986")
    public void createRenewToOpenSubscription() {
        log.info("Create Installment subscription with credit card where subscription type is auto-renew to open and being date is current date and frequency is WEEKLY with down payment.");
        setTestResultOnXRay("A30TP-21986");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_renewal_auto_to_open(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD, SUB_RENEWAL_TYPE,
    ...
    public void enterDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-21985");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,
                        SUB_CARD_NUMBER, SUB_CARD_HOLDER_NAME, SUB_CARD_EXPIRY_YEAR, SUB_CARD_POSTAL_CODE))
                .click_on_submit_btn();
    }

    @Test
    @Description("To be verify the subscription is created with all required data")
    @TmsLink("A30TP-21988")
    public void subscriptionCreationVerification() {
        log.info("To be verify the subscription is created with all required data.");
        setTestResultOnXRay("A30TP-21988");
        getPage(UnoAppSubscriptionPage.class)
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_WEEKLY_CARD,SUB_CARD_HOLDER_NAME));
    }
