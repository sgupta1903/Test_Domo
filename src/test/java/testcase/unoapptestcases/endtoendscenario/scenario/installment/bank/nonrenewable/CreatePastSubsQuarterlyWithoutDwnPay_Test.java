...
import static helper.DataProviderSection.PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK;
...
public class CreatePastSubsQuarterlyWithoutDwnPay_Test extends AbstractAutoUITest {
    ...
    public void setSectionData() {
        ...
        set_section_name(PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK);
    }
    ...
    @Test
    @Description("Displaying processor for company.")
    @TmsLink("A30TP-12691")
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create Installment non-renewal Subscription with down payment where begin date is current and frequency Weekly when payment is done by Bank account")
    @TmsLink("A30TP-19289")
    public void createNonRenewSubscription() {
        log.info("Create Installment non-renewal Subscription with down payment where begin date is current and frequency Weekly when payment is done by Bank account.");
        setTestResultOnXRay("A30TP-19289");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK,
    ...
    public void enterDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-8477");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_QUARTERLY_BANK,SUB_TYPE));
    }
