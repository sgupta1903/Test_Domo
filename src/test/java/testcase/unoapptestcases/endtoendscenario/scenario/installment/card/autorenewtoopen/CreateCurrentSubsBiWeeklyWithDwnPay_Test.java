...
import static helper.DataProviderSection.CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD;
...
@Issue("A30TP-21938")
@Slf4j
public class CreateCurrentSubsBiWeeklyWithDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD);
    }
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,
                        SUB_DOWNPAYMENT));

    }

    @Test
    @Description("Enter Installment subscription with credit card where subscription type is auto -renew to open and begin date is current date and frequency is BI-WEEKLY with down payment where expiration date and first due date is future date.")
    @TmsLink("A30TP-32268")
    public void createRenewToOpenSubscription() {
        log.info("Installment subscription with credit card where subscription type is auto -renew to open and begin date is current date and frequency is BI-WEEKLY with down payment where expiration date and first due date is future date.");
        setTestResultOnXRay("A30TP-32268");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_renewal_auto_to_open(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD, SUB_RENEWAL_TYPE,
    ...
    public void enterCardDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-32278");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_BIWEEKLY_CARD,SUB_CARD_HOLDER_NAME));
    }
