...
import static helper.DataProviderSection.CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK;
import static util.UtilityGeneric.set_section_name;

@Issue("A30TP-7785")
@Slf4j
public class CreateCurrentSubSemiAnnuallyWithoutDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK);
    }
    ...
    public void logoutPG() {
        log.info("Exit from Payment Gateway and switch billing.");
    ...
    public void createSubsWithOneMember() {
        ...
        setTestResultOnXRay("A30TP-8955");
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create Installment auto-renewal to open Subscription with down payment where begin date is current and frequency semi annually when payment is done by Bank account")
    @TmsLink("A30TP-21001")
    public void createRenewToOpenSubscription() {
        log.info("Create Installment auto-renewal to open Subscription with down payment where begin date is current and frequency semi annually when payment is done by Bank account.");
        setTestResultOnXRay("A30TP-21001");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_renewal_auto_to_open(getTestData(SUBSCRIPTION_INI, CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK, SUB_RENEWAL_TYPE,
    ...
    public void enterDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-21007");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI,CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITHOUT_DWNPAY_AUTORENEWTOOPEN_SEMIANNUALLY_BANK,SUB_ACCOUNT_HOLDER_NAME));
