...
import static helper.DataProviderSection.CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD;
...
@Issue("A30TP-21945")
@Slf4j
public class CreateCurrentSubsMonthlyWithDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(DataProviderSection.CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD);
    }
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,
                        SUB_DOWNPAYMENT));
    }
    ...
    public void enterDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-22029");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,
                        SUB_EXPIRY_DATE, SUB_DURATION));
    }

    @Test
    @Description("Create Installment subscription with credit card where subscription type is auto -renewal to open and begin date is current and expiration date is future and frequency is Monthly with down payment.")
    @TmsLink("A30TP-22030")
    public void subscriptionCreationVerification() {
        log.info("Create Installment subscription with credit card where subscription type is auto -renewal to open and begin date is current and expiration date is future and frequency is Monthly with down payment.");
        setTestResultOnXRay("A30TP-22030");
        getPage(UnoAppSubscriptionPage.class)
                .enter_renewal_auto_to_open(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD, SUB_RENEWAL_TYPE,
                        SUB_RENEWAL_DATE, SUB_RENEWAL_FREQUENCY))
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_MONTHLY_CARD,SUB_CARD_HOLDER_NAME));
    }
