...
import static helper.DataProviderSection.CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD;
...
@Issue("A30TP-21933")
@Slf4j
public class CreateCurrentSubsAnnuallyWithDwnPay_Test extends AbstractAutoUITest {
    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay(" A30TP-14514");
        set_section_name(CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD);
    }
    ...
    @Test
    @Description("Display the processors for company.")
    @TmsLink("A30TP-12691")
    public void selectingProcessor() {
        log.info("Display the processors for company.");
    ...
    public void configuringMerchantForVantivProcessor() {
        log.info("Creating a merchant processor for the location.");
    ...
    @Test
    @Description("Exit from Payment Gateway and switch to uno app.")
    @TmsLink("A30TP-12693")
    public void logoutPg() {
        log.info("Exit from Payment Gateway and switch to uno app");
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,
                        SUB_DOWNPAYMENT));

    }

    @Test
    @Description("Create Installment subscription with credit card where subscription type is auto -renewal to open and begin date is current date and frequency is Annually with down payment where expiration and first due date is future.")
    @TmsLink("A30TP-32323")
    public void createRenewToOpenSubscription() {
        log.info("Create Installment subscription with credit card where subscription type is auto -renewal to open and begin date is current date and frequency is Annually with down payment where expiration and first due date is future.");
        setTestResultOnXRay("A30TP-32323");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,
                        SUB_EXPIRY_DATE, SUB_DURATION))
                .enter_renewal_auto_to_open(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD, SUB_RENEWAL_TYPE,
    ...
    public void enterCardDetailsForSubscription() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,
                        SUB_CARD_NUMBER, SUB_CARD_HOLDER_NAME, SUB_CARD_EXPIRY_YEAR, SUB_CARD_POSTAL_CODE))
                .click_on_submit_btn();
    }

    @Test
    @Description("Verify the subscription created successfully.")
    @TmsLink("A30TP-11865")
    public void verifySubscriptionCreated() {
        log.info("Verifying the subscription created successfully with valid data.");
        setTestResultOnXRay("A30TP-11865");
        getPage(UnoAppSubscriptionPage.class)
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_AUTORENEWTOOPEN_ANNUALLY_CARD,SUB_CARD_HOLDER_NAME));
    }
