...
import static helper.DataProviderSection.CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK;
...
public class CreateCurrentSubsAnnuallyWithDwnPay_Test extends AbstractAutoUITest {
    ...
    public void setSectionData() {
        ...
        set_section_name(CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK);
    }
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create Installment non-renewal Subscription with down payment where begin date is current and frequency Annually when payment is done by Bank account")
    @TmsLink("A30TP-14191")
    public void createNonRenewSubscription() {
        log.info("Create Installment non-renewal Subscription with down payment where begin date is current and frequency Annually when payment is done by Bank account.");
        setTestResultOnXRay("A30TP-14191");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,
    ...
    public void enterDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-9547");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_ANNUALLY_BANK,SUB_ACCOUNT_HOLDER_NAME));
