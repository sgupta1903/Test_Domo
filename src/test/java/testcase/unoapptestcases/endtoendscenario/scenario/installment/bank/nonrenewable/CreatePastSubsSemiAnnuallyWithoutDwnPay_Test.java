...
import static helper.DataProviderSection.PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK;
...
public class CreatePastSubsSemiAnnuallyWithoutDwnPay_Test extends AbstractAutoUITest {
    ...
    public void setSectionData() {
        ...
        set_section_name(PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK);
    }
    ...
    public void createMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create Installment subscription with payment method type as 'Bank_Account' when begin,first due date is past and expiration date is future where frequency is Semi-Annually when renewal type is Non-renewal Agreement")
    @TmsLink("A30TP-14489")
    public void createNonRenewSubscription() {
        log.info("Entering the details related to Subscription schedule");
        setTestResultOnXRay("A30TP-14489");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,
    ...
    public void enterPaymentDetailsForSubscription() {
        log.info("Entering the details of card account for member.");
        setTestResultOnXRay("A30TP-10860");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI,PAST_WITHOUT_DWNPAY_NONRENEW_SEMIANNUALLY_BANK,SUB_ACCOUNT_HOLDER_NAME));
