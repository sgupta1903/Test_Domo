...
import static helper.DataProviderSection.CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK;
...
public class CreateCurrentSubsWeeklyWithDwnPay_Test extends AbstractAutoUITest {
    ...
    public void setSectionData() {
        ...
        set_section_name(CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK);
    }
    ...
    public void logoutFromPg() {
        log.info("Logging out from the Payment gateway");
    ...
    public void createSubsWithOneMember() {
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,
                        SUB_DOWNPAYMENT));
    }

    @Test
    @Description("Create installment non-renewable agreement subscription with down payment where frequency as weekly and begin date as Current date when payment is done by bank account")
    @TmsLink("A30TP-11776")
    public void createOpenEndedSubscription() {
        log.info("Create installment non-renewable agreement subscription with down payment where frequency as weekly and begin date as Current date when payment is done by bank account");
        setTestResultOnXRay("A30TP-11776");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT))
                .enter_subscription_schedule_information_installment(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,
    ...
    public void enterDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-11775");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_bank_account(getTestData(SUBSCRIPTION_INI, CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,SUB_TYPE));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                .click_payments_tab()
                .verify_wallet_payment_method_bank(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,PAYMENT_METHOD_BANK))
                .verify_wallet_bank_account_holder_name(getTestData(SUBSCRIPTION_INI,CURRENT_WITH_DWNPAY_NONRENEW_WEEKLY_BANK,SUB_ACCOUNT_HOLDER_NAME));
