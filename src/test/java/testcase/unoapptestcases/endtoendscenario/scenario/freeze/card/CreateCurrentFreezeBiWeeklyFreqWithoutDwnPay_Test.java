package testcase.unoapptestcases.endtoendscenario.scenario.freeze.card;
...
@Issue("A30TP-36799")
@Slf4j
public class CreateCurrentFreezeBiWeeklyFreqWithoutDwnPay_Test extends AbstractAutoUITest {

    @BeforeClass
    public void setSectionData() {
        log.info("Setting data for the test execution");
        setTestResultOnXRay("A30TP-14514");
        set_section_name(FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD);
    }
    ...
    @Test
    @Description("Creating VANTIV processor configuration for the selected location for Payment Gateway.")
    @TmsLink("A30TP-20722")
    public void creatingMerchantVantivProcessor() {
        log.info("Creating VANTIV processor configuration for the selected location for Payment Gateway");
    ...
    public void createSubsWithOneMember() {
        ...
        setTestResultOnXRay("A30TP-8331");
        ...
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_downpayment(getTestData(SUBSCRIPTION_INI,FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD,
                        SUB_DOWNPAYMENT));
    }
    ...
    public void enterDetailsForSubscription() {
        ...
        setTestResultOnXRay("A30TP-36633");
        getPage(UnoAppSubscriptionPage.class)
                .enter_subscription_schedule_information(getTestData(SUBSCRIPTION_INI, FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD,
                        SUB_TYPE, SUB_DUE_DATE, ITEM_NAME, SUB_BEGIN_DATE, SUB_PAY_FREQUENCY, SUB_PAY_AMOUNT));
    }

    @Test
    @Description("Enter Payment details and click on submit button.")
    @TmsLink("A30TP-32325")
    public void enterCardDetailsForSubscription() {
        log.info("Entering the details of credit and click on submit button.");
        setTestResultOnXRay("A30TP-32325");
        getPage(UnoAppSubscriptionPage.class)
                .enter_payment_information_credit_card(getTestData(SUBSCRIPTION_INI, FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD,
                        SUB_CARD_NUMBER, SUB_CARD_HOLDER_NAME, SUB_CARD_EXPIRY_YEAR, SUB_CARD_POSTAL_CODE))
                .click_on_submit_btn();
    }
    @Test
    @Description("Create Freeze subscription when subscription type is open ended where begin date and due date is current and frequency is weekly and payment method as bank account.")
    @TmsLink("A30TP-36634")
    public void createSubscription() {
        log.info("Create Freeze subscription when subscription type is open ended where begin date and due date is current and frequency is weekly and payment method as bank account.");
        setTestResultOnXRay("A30TP-36634");
        getPage(UnoAppSubscriptionPage.class)
    ...
    public void verifySubscriptionTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_subscription_frequency(getTestData(SUBSCRIPTION_INI,FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD,SUB_PAY_FREQUENCY))
                .verify_subscription_amenity(getTestData(SUBSCRIPTION_INI,FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD,ITEM_NAME))
                .verify_subscription_type(getTestData(SUBSCRIPTION_INI,FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD,SUB_TYPE));
    }

    @Test
    @Description("Click on subscription list and enter the freeze details")
    @TmsLink("A30TP-36494")
    public void freezeSubscription(){
        log.info("Click on subscription list and enter the freeze details");
        setTestResultOnXRay("A30TP-36494");
        getPage(UnoAppMemberDetailPage.class)
                .select_subscription_list()
                .click_freeze_button();
    }

    @Test
    @Description("Enter the freeze subscription details")
    @TmsLink("A30TP-36502")
    public void enterFreezeDetails(){
        log.info("Enter the freeze subscription details");
        setTestResultOnXRay("A30TP-36502");
        getPage(UnoAppMemberDetailPage.class)
                .enter_freeze_type(getTestData(UNOAPP_INI,FREEZE_SECTION,FREEZE_TYPE))
                .enter_freeze_end_date(getTestData(UNOAPP_INI,FREEZE_SECTION,FREEZE_END_DATE))
                .enter_freeze_start_date(getTestData(UNOAPP_INI,FREEZE_SECTION,FREEZE_START_DATE))
                .click_freeze_save_button();
    }
    @Test
    @Description("Verifying the freeze subscription details")
    @TmsLink("A30TP-36506")
    public void verifyFreezeSubscription(){
        log.info("Click on subscription list and enter the freeze details");
        setTestResultOnXRay("A30TP-36506");
        getPage(UnoAppMemberDetailPage.class)
                .click_close_drawer_dialog()
                .verify_freeze_subscription_status(getTestData(UNOAPP_INI,FREEZE_SECTION,FREEZE_STATUS));
    }
    ...
    public void verifyWalletTab() {
        ...
        getPage(UnoAppMemberDetailPage.class)
                ...
                .verify_wallet_card_account_holder_name(getTestData(SUBSCRIPTION_INI,FREEZE_CURRENT_OPENENDED_BIWEEKLY_WITHOUT_DWNPAY_CARD,SUB_CARD_HOLDER_NAME));
